...
#' 
#' @param dice
#' The observed dice value (as given by `CompDICE`).
#'
#' @param nMC
...
#' @export
#'
CompPrimingPval <- function(data.m, dice, nMC=1000, ncores=4){

  idx.l <- as.list(1:nMC);
  mcl.o <- mclapply(idx.l,doPrimedPRL,data.m,mc.cores=ncores);
  null.v <- unlist(mcl.o);
  pval <- length(which(null.v < dice))/nMC;
